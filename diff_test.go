...
// mutate applies one string replacement and fails the test if nothing changed,
// so a stale pattern cannot silently turn a test into a no-op.
func mutate(t *testing.T, base, from, to string) string {
	t.Helper()
	out := strings.Replace(base, from, to, 1)
	if out == base {
		t.Fatalf("mutate %q -> %q did not apply", from, to)
	}
	return out
}
...
func TestDiffReportsExactlyTheChangedElement(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			rep := diffStrings(t, base, mutate(t, base, tc.from, tc.to), DiffOptions{})
...
func TestDiffAddedSegmentWithinATransaction(t *testing.T) {
	...
	with := mutate(t, base, "TRN*1*20260220BP0001*1443322110~\n",
		"TRN*1*20260220BP0001*1443322110~\nREF*EV*BP835~\n")
	with = mutate(t, with, "SE*13*0001~", "SE*14*0001~")
...
func TestDiffTransactionTypeChangeIsAddAndRemove(t *testing.T) {
	...
	base := string(readFixture(t, "diff_base_835.x12"))
	changed := mutate(t, base, "ST*835*0002~", "ST*837*0002~")
